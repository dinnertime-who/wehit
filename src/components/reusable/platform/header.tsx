  ...
  return (
    <header className="flex flex-col sticky top-0 z-50 h-(--header-height) w-full border-b bg-white/80 backdrop-blur-lg">
      <div className="app-container flex items-center justify-between px-4 h-14 md:h-20">
        ...
        {/* 우측 액션 버튼 */}
        <div className="flex items-center text-base font-medium gap-x-5 break-keep">
          ...
          <Link
            ...
            className={cn(
              ...
              "hidden md:flex rounded-full font-black py-1.5 h-auto text-[15px]",
            )}
          >
            회원가입
          </Link>
      ...
      <div className="flex-1 app-container flex gap-x-8 items-end justify-start text-heading-5 translate-y-px">
        {BANNER_TAGS.map((tag) => (
          <button
            key={tag.value}
            type="button"
            className="py-2.5 cursor-pointer border-b-3 transition-all duration-200 border-transparent hover:border-pink-600 data-[selected=true]:border-pink-600"
